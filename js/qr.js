var qrcode = function() {
	var qrcode = function(typeNumber, errorCorrectLevel) {
		...
		var makeImpl = function(test, maskPattern) {
			...
			if (_dataCache == null) {
				_dataCache = createData(_typeNumber, _errorCorrectLevel,
						_dataList);
			}
		...
		var createBytes = function(buffer, rsBlocks) {
			...
			for (var r = 0; r < rsBlocks.length; r += 1) {
				...
				for (var i = 0; i < ecdata[r].length; i += 1) {
					...
					ecdata[r][i] = (modIndex >= 0) ? modPoly.getAt(modIndex)
							: 0;
				}
			}
		...
		var createData = function(typeNumber, errorCorrectLevel, dataList) {
			...
			for (var i = 0; i < dataList.length; i += 1) {
				...
				buffer.put(data.getLength(), QRUtil.getLengthInBits(data
						.getMode(), typeNumber));
				data.write(buffer);
			}
		...
		_this.isDark = function(row, col) {
			if (row < 0 || _moduleCount <= row || col < 0
					|| _moduleCount <= col) {
	...
	var QRUtil = function() {
		var PATTERN_POSITION_TABLE = [ [], [ 6, 18 ], [ 6, 22 ], [ 6, 26 ],
		...
		var G15 = (1 << 10) | (1 << 8) | (1 << 5) | (1 << 4) | (1 << 2)
				| (1 << 1) | (1 << 0);
		var G18 = (1 << 12) | (1 << 11) | (1 << 10) | (1 << 9) | (1 << 8)
				| (1 << 5) | (1 << 2) | (1 << 0);
...
var update_qrcode = function() {
	var text = document.forms[0].elements['$codigoHTML'].value.replace(
			/^[\s\u3000]+|[\s\u3000]+$/g, '');
